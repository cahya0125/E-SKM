		...
		<header class="flex h-16 items-center justify-between bg-white px-4 sm:px-6">
			<div>
				...
				<p class="mt-1 text-[10px] text-slate-400">
					Selamat datang, {{ Auth::user()->name }} · {{ now()->locale('id')->translatedFormat('l, d F Y') }}
				</p>
			</div>

			<div class="flex items-center gap-4">
				<span class="relative inline-flex text-slate-500" aria-label="Notifikasi">
					<i class="fa-solid fa-bell h-5 w-5 text-center" aria-hidden="true"></i>
					<i class="absolute -right-0.5 top-0 h-1 w-1 rounded-full bg-[#c43b2d]"></i>
				</span>

				<span class="grid h-8 w-8 place-content-center rounded-full bg-[#c43b2d] text-xs font-semibold text-white">
					A
				</span>
			</div>
		</header>
