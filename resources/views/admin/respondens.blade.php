        ...
        <section class="max-h-[calc(100vh-24px)] w-full max-w-xl overflow-hidden rounded-xl bg-white shadow-2xl" @click.outside="modalOpen = false" role="dialog" aria-modal="true">
            ...
            <div class="max-h-[calc(100vh-100px)] overflow-y-auto p-4 sm:p-5">
                <div x-show="detailMode" class="space-y-4 text-xs">
                    <div class="grid grid-cols-1 gap-3 rounded-lg border border-red-200 bg-red-50/70 p-3 sm:grid-cols-[1fr_auto_auto]"><div><span class="block text-[9px] uppercase text-slate-400">Nilai IKM Responden</span><strong class="text-2xl leading-none text-[#c43b2d]" x-text="activeResponden?.ikm ?? '-' "></strong></div><div><span class="block text-[9px] text-slate-400">Jenis Layanan</span><strong x-text="activeResponden?.jenisLayanan ?? '-' "></strong></div><div><span class="block text-[9px] text-slate-400">Status Dampak</span><strong x-text="activeResponden?.impact ?? '-' "></strong></div></div>
                ...
                <form x-show="!detailMode" class="space-y-4 text-[10px] font-semibold text-slate-700" @submit.prevent="save">
                    <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
                        ...
                        <label>Jenis Kelamin<select class="mt-1 block h-8 w-full rounded-md border border-slate-200 bg-white px-2 text-xs font-normal outline-0" x-model="form.jenis_kelamin"><option value="L">Laki-laki</option><option value="P">Perempuan</option></select></label>
                        ...
                        <label>Pekerjaan<input class="mt-1 block h-8 w-full rounded-md border border-slate-200 px-2 text-xs font-normal outline-0" x-model="form.pekerjaan" required></label>
...
function respondenManagement() {
    return {
        ...
        get detailItems() { const item = this.activeResponden ?? {}; return [{ label: 'Jenis Kelamin', value: item.jenisKelamin }, { label: 'Usia', value: item.usia ? `${item.usia} tahun` : '-' }, { label: 'Pendidikan', value: item.pendidikan }, { label: 'Pekerjaan', value: item.pekerjaan }, { label: 'No. HP', value: item.noHp }, { label: 'Kelurahan', value: item.kelurahan }]; },
